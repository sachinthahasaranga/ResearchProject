...
exports.createPaper = async (req, res) => {
    try {
        const { paperTitle, recommendedAge, difficultyLevel, totalTime, categoryId } = req.body;
...
exports.getPapers = async (req, res) => {
    try {
        const papers = await Paper.find().populate("difficultyLevel").populate("categoryId", "categoryName")
...
exports.getPaperById = async (req, res) => {
    try {
        const paper = await Paper.findById(req.params.id).populate("difficultyLevel").populate("createdBy", "username email").populate("categoryId", "categoryName");
...
exports.updatePaper = async (req, res) => {
    try {
        ...
        const updatedPaper = await Paper.findByIdAndUpdate(
            req.params.id,
            { paperTitle, recommendedAge, difficultyLevel, totalTime, categoryId },
            { new: true }
        );
